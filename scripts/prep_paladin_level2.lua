...
local function prepPaladin(nodeClassLevel)
	if DB.isOwner(nodeClassLevel) then
		...
		if nPalLvl and nPalLvl > 1 then -- if PC is at least a 2nd level paladin
			...
			if not sFort2Stat or not sRef2Stat or not sWill2Stat or sFort2Stat == '' or sRef2Stat == '' or sWill2Stat == '' then
				bStatChanged = true
			end
...
function onInit() DB.addHandler(DB.getPath('charsheet.*.classlevel'), 'onUpdate', prepPaladin) end
