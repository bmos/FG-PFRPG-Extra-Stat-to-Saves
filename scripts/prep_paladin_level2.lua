		...
		if nPalLvl and nPalLvl > 1 then -- if PC is at least a 2nd level paladin
			...
			if bStatChanged and (not Session.IsHost or bGMOnlyConnected) then -- if any ability2 are at default values, set them to charisma
				...
				if not sFort2Stat or sFort2Stat == "" then
					DB.setValue(nodeChar, "saves.fortitude.ability2", "string", "charisma")
				end
				if not sRef2Stat or sRef2Stat == "" then
					DB.setValue(nodeChar, "saves.reflex.ability2", "string", "charisma")
				end
				if not sWill2Stat or sWill2Stat == "" then
					DB.setValue(nodeChar, "saves.will.ability2", "string", "charisma")
				end
			end
		end
	end
