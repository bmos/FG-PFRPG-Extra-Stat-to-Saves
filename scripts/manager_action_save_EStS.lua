...
local function getRoll_new(rActor, sSave, ...)
    ...
    if rActor then
        ...
        if ActorManager.isPC(rActor) then
            ...
            sAbility2 =
                DB.getValue(nodePC, "saves." .. sSave .. ".ability2", "");
        elseif nodeCT then
...
local function modSave_new(rSource, rTarget, rRoll, ...)
    ...
    if rSource then
        ...
        local nBonusStat2, nBonusEffects2 =
            ActorManager35E.getAbilityEffectsBonus(rSource, sActionStat2);
        ...
        if bEffects then
            local sEffects;
            local sMod =
                DiceManager.convertDiceToString(aAddDice, nAddMod, true);
            if sMod ~= "" then
                sEffects =
                    "[SECOND MOD " .. Interface.getString("effects_tag") .. " " ..
                        sMod .. "]";
            else
                sEffects =
                    "[SECOND MOD " .. Interface.getString("effects_tag") .. "]";
